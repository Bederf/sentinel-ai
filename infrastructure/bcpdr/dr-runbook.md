...
### 3.1 Full VPS Recovery (Snapshot)

**When to use:** Complete VM failure, unresponsive VM, corrupted OS. Snapshot available from provider.

**Pre-requisites:**
- Provider dashboard access credentials
- SSH key backup
- Application repository cloned (`git clone <repo>`)
- Infra configs from `infrastructure/bootstrap.sh`
- Encrypted secrets bundle from offsite storage (see 3.6)
...
STEP 3: Verify base system
  ...
  b) Verify: hostname, disk space, memory
  c) Clone the application repository:
     $ git clone <repo-url> /opt/bms-intelligence
  d) If using bootstrap path (no snapshot):
     $ cd /opt/bms-intelligence/infrastructure
     $ sudo ./bootstrap.sh
  e) If snapshot was recent, Docker and services may auto-start

STEP 4: Restore secrets from encrypted bundle
  a) Retrieve secrets bundle from offsite storage
  b) See section 3.6 (Secrets Bundle Recovery) for decrypt procedure
  c) Verify .env files restored

STEP 5: Restore infrastructure services
  a) Systemd units are in infrastructure/systemd/:
     $ cp infrastructure/systemd/*.service /etc/systemd/system/
     $ systemctl daemon-reload
  b) Cloudflare tunnel:
     $ cp infrastructure/cloudflared/config.yml /etc/cloudflared/
  c) Caddy:
     $ cp infrastructure/caddy/Caddyfile /opt/bms-intelligence/
  d) Mosquitto:
     $ cp infrastructure/mosquitto/* /etc/mosquitto/conf.d/

STEP 6: Start services
  $ systemctl start sentinel-backend sentinel-frontend sentinel-caddy sentinel-bms-tunnel mosquitto
  $ sleep 10
...
### 3.6 Secrets Bundle Recovery

**When to use:** Full secrets loss — `.env` files, Cloudflare tunnel credentials, MQTT passwords, WireGuard keys, SSH host keys all gone. Requires the encrypted secrets bundle from offsite storage plus its decryption key.

**Prerequisites:**
- Encrypted secrets bundle file from offsite storage
- Age decryption key (separately stored from the bundle)

**Procedure:**

```
STEP 1: Retrieve secrets bundle from offsite storage
  Destination depends on configured offsite location:
  - Backblaze B2 / S3:  aws s3 cp s3://<bucket>/sentinel-secrets-<date>.tar.age ./
  - SCP:                 scp user@backup-host:/path/sentinel-secrets-<date>.tar.age ./
  - Password manager:    Download manually

STEP 2: Decrypt the bundle
  $ age --decrypt -i /path/to/secrets-key.txt \
      -o secrets.tar sentinel-secrets-<date>.tar.age
  $ tar -xvf secrets.tar -C /

STEP 3: Verify critical files restored
  $ ls -la /opt/bms-intelligence/backend/.env
  $ ls -la /etc/cloudflared/sentinel-bms-credentials.json
  $ ls -la /etc/mosquitto/passwd

STEP 4: Restart affected services
  $ systemctl daemon-reload
  $ systemctl restart sentinel-backend sentinel-frontend sentinel-bms-tunnel
  $ systemctl restart mosquitto cloudflared caddy

STEP 5: Verify
  $ curl -s http://localhost:9095/api/health
```

**Note:** The secrets bundle workflow lives at `infrastructure/secrets-bundle.sh`.
Run `./secrets-bundle.sh encrypt` after any secret rotation to keep the offsite bundle current.
Never store decryption keys on the same host as the encrypted bundle.
