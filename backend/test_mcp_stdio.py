...
def test_stdio_server():
    ...
    try:
        ...
        print(
            f"  OK: Server: {response['result']['serverInfo']['name']} v{response['result']['serverInfo']['version']}"
        )
        print()
        ...
        request = {
            ...
            "id": 3,
            "method": "tools/call",
            "params": {"name": "get_buildings", "arguments": {}},
        }
        proc.stdin.write(json.dumps(request) + "\n")
        proc.stdin.flush()
        response_line = proc.stdout.readline()
        ...
        request = {"jsonrpc": "2.0", "id": 4, "method": "unknown_method"}
        proc.stdin.write(json.dumps(request) + "\n")
        proc.stdin.flush()
        response_line = proc.stdout.readline()
        response = json.loads(response_line)
