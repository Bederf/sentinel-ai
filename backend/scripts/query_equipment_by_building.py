...
# 4. Check equipment with missing building_id
try:
    print("\n4. EQUIPMENT WITH POTENTIALLY MISSING BUILDING_ID")
    print("-" * 70)
    equip = client.table("equipment").select("id, code, name, building_id").is_("building_id", "null").execute()
    orphan_count = len(equip.data) if equip.data else 0
    print(f"   Orphaned equipment (null building_id): {orphan_count}")
...
try:
    equipment = repo.get_by_building_code("site-002")
    print(f"   repo.get_by_building_code('site-002'): {len(equipment)} items")
    if equipment:
