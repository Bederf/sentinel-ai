...
LARGE_EQUIPMENT_TYPES = [
    "chiller", "ahu", "pump", "generator",
    "cooling_tower", "ups", "bess",
]
...
def run_backfill():
    ...
    for r in records:
        src = "original" if "original" in r["confidence_notes"] else r["confidence_notes"].split("from: ")[1].split(".")[0]
    ...
    for rec in records:
        result = supabase.table("equipment_service_history").upsert(
            rec, on_conflict="site_id,equipment_code"
        ).execute()
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "equipment_code", "equipment_type", "commissioning_date",
            "commissioning_date_source", "manufacturer", "baseline_calculation_method",
        ])
        writer.writeheader()
    ...
    print(f"  Baseline calculation method: age_only (all)")
    print(f"  Health score confidence: NULL (pending baseline calculation)")
    print()
