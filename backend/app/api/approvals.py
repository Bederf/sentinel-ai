...
async def approve_recommendation(
    ...
    try:
        if not request:
            raise HTTPException(
                status_code=400,
                detail="Request body required with approved_by field"
            )
        ...
        if not result.success:
            ...
            raise HTTPException(
                ...
                detail=result.error_message or "Approval execution failed"
            )
...
async def reject_recommendation(
    ...
    try:
        if not request:
            raise HTTPException(
                status_code=400,
                detail="Request body required with rejected_by and reason"
            )
        ...
        if not result.success:
            raise HTTPException(
                status_code=400,
                detail=result.error_message or "Rejection failed"
            )
...
async def get_approval_status(
    recommendation_id: str = Path(..., description="ID of recommendation"),
    auth: AuthContext = Depends(require_auth)
) -> ApprovalStatus:
...
async def rollback_approval(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to rollback recommendation: {str(e)}"
        )
