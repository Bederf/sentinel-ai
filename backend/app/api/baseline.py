...
async def capture_automated_baseline(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to capture automated baseline: {str(e)}"
        )
...
async def capture_element_baseline(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to capture element baseline: {str(e)}"
        )
...
async def get_active_element_baseline(
    ...
    if not element:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Element {element_id} not found for equipment {equipment_id}"
        )
    ...
    if not baseline:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No active baseline found for element {element_id}"
        )
