...
def _no_live_dali_data(site_id: str | None, reason: str | None = None) -> dict:
    return {
        "site_id": site_id,
        "data_source": "live",
...
async def get_live_dali_data(
    ...
    This endpoint is retained for DALI URL compatibility but delegates to the
    protocol-neutral LightingService. PIR values
...
async def get_building_occupancy(site_id: str = Query(None, description="Site ID")):
    ...
    if not zones:
        return _no_live_dali_data(requested_site_id, live_data.get("error"))
...
async def get_zone_lighting(zone_id: str):
    """Return live zone lighting data only; static zone examples are removed."""
    from app.services.lighting_service import get_lighting_service
...
@router.get("/stats")
async def get_dali_stats(site_id: str = Query(..., description="Site ID")):
