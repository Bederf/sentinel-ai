...
class ApprovalRequest(BaseModel):
    ...
    approved_by: str = Field(
        ...,
        description="User ID or name of person approving",
        min_length=1,
        max_length=255
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "approval_notes": "Peak demand response - urgent"
            }
        }
...
class RejectionRequest(BaseModel):
    ...
    rejected_by: str = Field(
        ...,
        description="User ID or name of person rejecting",
        min_length=1,
        max_length=255
    )
    reason: str = Field(
...
class ApprovalResponse(BaseModel):
    ...
    status: str = Field(
        ...,
        description="Result status: approved, rejected, executed, or failed"
    )
    executed_at: Optional[datetime] = Field(
        None,
        description="Timestamp when approved action was executed"
    )
    error_message: Optional[str] = Field(
        None,
        description="Error message if approval failed"
    )
    cov_verified: bool = Field(
        False,
        description="Whether COV feedback verified the device change"
    )
    execution_result: Optional[Dict[str, Any]] = Field(
        None,
        description="Detailed execution result data"
    )
...
class ApprovalStatus(BaseModel):
    ...
    approval_status: str = Field(
        ...,
        description="Current status: pending, approved, rejected, executed, or failed"
    )
    ...
    executed_at: Optional[datetime] = Field(
        None,
        description="When the approved action was executed"
    )
    rejection_reason: Optional[str] = Field(
        None,
        description="If rejected, the reason why"
    )
