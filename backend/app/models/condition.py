...
class ElementRUL(BaseModel):
    ...
    days_until_threshold: Optional[float] = Field(
        None,
        description="Predicted days until threshold reached (None if stable/improving)"
    )
...
class EquipmentRUL(BaseModel):
    ...
    days_until_first_threshold: Optional[float] = Field(
        None,
        description="Days until first element reaches threshold (None if all stable)"
    )
    ...
    recommended_service_window: Optional[str] = Field(
        None,
        description="Service scheduling recommendation (e.g., 'within 30 days')"
    )
...
class ServiceRecommendation(BaseModel):
    ...
    estimated_days_remaining: Optional[float] = Field(
        None,
        description="Estimated days before failure (None if unknown)"
    )
...
class AssetUtilization(BaseModel):
    ...
    utilization_percent: float = Field(
        ...
        ge=0,
        le=100,
        description="Percentage of usable life consumed (0=new, 100=end of life)"
    )
    remaining_percent: float = Field(
        default=100.0,
        ge=0,
        le=100,
        description="Percentage of usable life remaining"
    )
    status: UtilizationStatus = Field(
        default=UtilizationStatus.HEALTHY,
        description="Health status based on utilization"
    )
...
class ServiceWindow(BaseModel):
    ...
    elements_driving: List[str] = Field(
        default_factory=list,
        description="Element names driving the service need"
    )
    cost_impact: str = Field(
        default="low",
        description="Cost impact of timing: low (>90 days), medium (30-90), high (<30 days)"
    )
...
class MaintenanceCostComparison(BaseModel):
    ...
    equipment_id: str = Field(..., description="Equipment identifier")
    equipment_type: Optional[str] = Field(None, description="Type of equipment")
    fixed_schedule_services_per_year: int = Field(
        ...,
        description="Number of services per year on fixed schedule"
    )
    ...
    savings_percent: float = Field(
        default=0.0,
        description="Cost savings percentage (positive = condition-based is cheaper)"
    )
    recommendation: str = Field(
        default="",
        description="Human-readable recommendation based on cost comparison"
    )
...
class OptimizeScheduleRequest(BaseModel):
    ...
    equipment_ids: Optional[List[str]] = Field(
        None,
        description="Equipment IDs to optimize (all if None)"
    )
    fixed_interval_days: int = Field(
        default=90,
        ge=7,
        le=365,
        description="Fixed-schedule interval in days for comparison"
    )
...
class OptimizedSchedule(BaseModel):
    ...
    equipment_ids: List[str] = Field(
        default_factory=list,
        description="Equipment IDs included in schedule"
    )
    schedule: List[ServiceWindow] = Field(
        default_factory=list,
        description="Sorted service windows (by optimal_date)"
    )
    total_equipment: int = Field(default=0, description="Total equipment analyzed")
    equipment_needing_service: int = Field(
        default=0,
        description="Equipment with active service windows"
    )
    cost_comparison_summary: str = Field(
        default="",
        description="Summary of cost savings from condition-based approach"
    )
