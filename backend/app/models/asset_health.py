...
class AssetHealthBaseline(BaseModel):
    ...
    last_baseline_at: str | None = Field(None, description="ISO timestamp of the active baseline capture date")
    ...
    confidence: str | None = Field(None, description="Data quality confidence: 'high' | 'medium' | 'low'")
    trend_7d: float | None = Field(None, description="7-day health score slope (points per day)")
    trend_30d: float | None = Field(None, description="30-day health score slope (points per day)")
    assessment_state: str | None = Field(None, description="'normal' | 'degraded_data' | 'insufficient_data'")
