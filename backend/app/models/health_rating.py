...
class DailyRollup(BaseModel):
    ...
    score_min: Optional[float] = Field(None, description="Minimum health score for the day")
    score_max: Optional[float] = Field(None, description="Maximum health score for the day")
    score_avg: Optional[float] = Field(None, description="Average health score for the day")
    status_mode: Optional[str] = Field(None, description="Most common status for the day")
    confidence_mode: Optional[str] = Field(None, description="Most common confidence for the day")
...
class AssetHealthSummaryItem(BaseModel):
    ...
    trend_7d: Optional[float] = Field(None, description="7-day score trend (positive = improving)")
    trend_30d: Optional[float] = Field(None, description="30-day score trend")
    has_active_baseline: bool = Field(False, description="Whether equipment has an active baseline")
    last_baseline_at: Optional[str] = Field(None, description="ISO timestamp of active baseline")
    max_deviation_percent_24h: Optional[float] = Field(None, description="Max deviation % in last 24 hours")
...
class RecomputeRequest(BaseModel):
    ...
    equipment_id: Optional[str] = Field(None, description="Single equipment to recompute")
    site_id: Optional[str] = Field(None, description="Recompute all equipment at a site")
