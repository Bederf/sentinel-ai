...
class EquipmentMetadataRepository:
    ...
    def get_notes_history(self, equipment_id: str, limit: int = 20) -> list:
        ...
        response = self.client.table('equipment_notes_history').select(
            '*'
        ).eq('equipment_id', eq['id']).order(
    ...
    def search_by_network_info(self, search_key: str, search_value: str) -> list:
        ...
        response = self.client.table('equipment').select(
            'id, code, name, type, network_info, device_info'
        ).filter(
            'network_info', 'cs', json.dumps({search_key: search_value})
        ).execute()
