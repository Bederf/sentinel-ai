...
class GeneratorRepository:
    ...
    def get_tank_by_id(self, tank_id: str) -> Optional[Dict[str, Any]]:
        ...
        response = self.client.table('diesel_tanks').select("*").eq(
            'tank_id', tank_id
        ).execute()
    ...
    def upsert_tank(self, tank_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self.client.table('diesel_tanks').upsert(
            tank_data,
            on_conflict='tank_id'
        ).execute()
    ...
    def delete_tank(self, tank_id: str) -> bool:
        ...
        response = self.client.table('diesel_tanks').delete().eq(
            'tank_id', tank_id
        ).execute()
    ...
    def update_tank_level(self, tank_id: str, level_liters: int, level_pct: float) -> Optional[Dict[str, Any]]:
        ...
        response = self.client.table('diesel_tanks').update({
    ...
    def get_group_by_id(self, group_id: str) -> Optional[Dict[str, Any]]:
        ...
        response = self.client.table('generator_groups').select("*").eq(
            'group_id', group_id
        ).execute()
    ...
    def get_group_uuid(self, group_id: str) -> Optional[str]:
        ...
        response = self.client.table('generator_groups').select('id').eq(
            'group_id', group_id
        ).execute()
    ...
    def get_tank_uuid(self, tank_id: str) -> Optional[str]:
        ...
        response = self.client.table('diesel_tanks').select('id').eq(
            'tank_id', tank_id
        ).execute()
    ...
    def upsert_group(self, group_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self.client.table('generator_groups').upsert(
            group_data,
    ...
    def delete_group(self, group_id: str) -> bool:
        ...
        response = self.client.table('generator_groups').delete().eq(
            'group_id', group_id
        ).execute()
        return len(response.data) > 0
    ...
    def update_group_status(
        ...
        response = self.client.table('generator_groups').update({
    ...
    def get_generators_by_group(self, group_id: str) -> List[Dict[str, Any]]:
        ...
        response = self.client.table('generators').select("*").eq(
            'group_id', group_uuid
        ).execute()
    ...
    def upsert_generator(self, gen_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self.client.table('generators').upsert(
            gen_data,
            on_conflict='generator_id'
        ).execute()
        return response.data[0] if response.data else {}
    ...
    def delete_generator(self, generator_id: str) -> bool:
        """Delete a generator."""
        response = self.client.table('generators').delete().eq(
            'generator_id', generator_id
        ).execute()
        return len(response.data) > 0
    ...
    def update_generator_status(
        ...
        response = self.client.table('generators').update({
            'status': status,
            'engine_running': engine_running,
            'on_load': on_load,
        }).eq('generator_id', generator_id).execute()
    ...
    def delete_all_by_building(self, building_code: str) -> Dict[str, int]:
        ...
        gen_response = self.client.table('generators').delete().eq(
            'building_id', building_uuid
        ).execute()

        # Delete groups
        grp_response = self.client.table('generator_groups').delete().eq(
            'building_id', building_uuid
        ).execute()

        # Delete tanks
        tank_response = self.client.table('diesel_tanks').delete().eq(
            'building_id', building_uuid
        ).execute()
