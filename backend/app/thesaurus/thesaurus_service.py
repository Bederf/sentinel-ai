...
def _build_result(entry: ThesaurusEntry, matched_phrase: str, score: int) -> ClassificationResult:
    ...
    return ClassificationResult(
        discipline="HVAC"
        if cat in (ComplaintCategory.TOO_HOT, ComplaintCategory.TOO_COLD, ComplaintCategory.STUFFY_AIR)
        else cat.value.title(),
...
class ThesaurusService:
    ...
    def classify(self, text: str) -> ClassificationResult | None:
        ...
        best_score = 0
        best_entry: ThesaurusEntry | None = None
        best_canonical: str = ""
...
# ----------------------------------------------------------------------------------
_service: ThesaurusService | None = None
...
def classify_complaint(text: str) -> dict | None:
