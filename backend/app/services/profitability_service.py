...
class ProfitabilityService:
    ...
    def calculate_portfolio_metrics(self, period_start: date, period_end: date) -> PortfolioMetrics:
        """
        ...
        for contract in contracts:
            profitability = self.calculate_contract_profitability(contract["id"], period_start, period_end)
        ...
        margin_pct = round((gross_margin / total_revenue * 100) if total_revenue > 0 else 0.0, 2)
    ...
    def calculate_contract_profitability(
        ...
        margin_pct = round((gross_margin / net_revenue * 100) if net_revenue > 0 else 0.0, 2)
        ...
        mom_change = self._calculate_mom_change(contract_id, period_start, period_end)
        ...
        for contract in contracts:
            try:
                ...
                if profitability.status == "loss":
                    ...
                    months_in_loss = self._count_consecutive_loss_months(contract["id"], period_end)
                    cumulative_loss = self._calculate_cumulative_loss(contract["id"], months_in_loss)
    ...
    def calculate_profitability_trends(self, contract_id: str, months: int = 12) -> List[ProfitabilityTrend]:
        ...
        for i in range(months):
            ...
            try:
                profitability = self.calculate_contract_profitability(contract_id, period_start, period_end)
    ...
    def generate_contract_report(
        ...
        assets = self.calculate_contract_asset_roi_list(contract_id, period_start, period_end, limit=asset_limit)
    ...
    def _get_clawbacks_for_period(self, contract_id: str, period_start: date, period_end: date) -> float:
        """
    ...
    def _get_contract_costs(self, contract_id: str, period_start: date, period_end: date) -> Dict[str, float]:
        ...
        """
        costs = {"labor": 0.0, "parts": 0.0, "subcontractor": 0.0, "callout": 0.0, "consumables": 0.0}
    ...
    def _count_consecutive_loss_months(self, contract_id: str, as_of_date: date) -> int:
    ...
    def _calculate_cumulative_loss(self, contract_id: str, months: int) -> float:
    ...
    def _is_profitable(self, contract_id: str, period_start: date, period_end: date) -> bool:
    ...
    def _calculate_contract_cost(self, contract_id: str, period_start: date, period_end: date) -> float:
