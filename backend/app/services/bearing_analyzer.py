...
class BearingAnalyzer:
    ...
    FAULT_SIGNATURES = {
        'imbalance': {
            'freq_ratio': 1.0,
            'description': 'Strong 1x peak - rotor imbalance'
        },
        'misalignment': {
            'freq_ratio': 2.0,
            'description': 'Strong 2x peak - shaft misalignment'
        },
        'looseness': {
            'freq_ratios': [0.5, 1.0, 2.0, 3.0],
            'description': 'Multiple harmonics - mechanical looseness'
        },
        'electrical': {
            'freq_ratio': 2.0,
            'sidebands': True,
    ...
    def analyze(
        ...
        eq_range = self.EQUIPMENT_FREQUENCY_RANGES.get(
            equipment_type,
            self.EQUIPMENT_FREQUENCY_RANGES['motor']
        )
        ...
        bearing_peaks = [
            ...
            if bearing_band[0] <= f <= bearing_band[1]
        ]
        ...
        if bearing_peaks:
            defect_result = self._check_bearing_defects(
                bearing_peaks, shaft_freq, peak_freqs
            )
