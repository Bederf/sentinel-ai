...
from app.models.solar import SolarInverter, SolarString, BESSContainer
...
class SolarPerformanceService:
    ...
    async def compare_inverter_peers(
        self, site_id: str
    ) -> List[InverterComparison]:
        ...
        for group_key, group_inverters in peer_groups.items():
            ...
            for inv in group_inverters:
                sy = (
                    inv.daily_yield_kwh / inv.rated_power_kva
                    if inv.rated_power_kva > 0 else 0
                )
            ...
            group_stdev = (
                statistics.stdev(yield_values) if len(yield_values) > 1 else 0
            )
            ...
            for rank, (inv, sy) in enumerate(yields, start=1):
                ...
                deviation = (
                    (sy - group_mean) / group_mean if group_mean > 0 else 0
                )
                ...
                if status != "normal" and group_mean > 0:
                    ...
                    cost_impact = (
                        lost_yield * AVERAGE_TARIFF_ZAR_KWH * 30
                    )  # Monthly estimate

                    if abs(deviation) > 0.15:
                        probable_cause = (
                            "Severe underperformance — possible MPPT tracker "
                            "failure or multiple string faults"
                        )
                    elif abs(deviation) > 0.10:
                        probable_cause = (
                            f"String fault on MPPT tracker — "
                            f"{abs(deviation)*100:.0f}% below peer group"
                        )
                    else:
                        probable_cause = (
                            "Minor underperformance — possible soiling, "
                            "partial shading, or connector degradation"
                        )

                results.append(InverterComparison(
    ...
    async def detect_string_anomalies(
        ...
        for inv in inverters:
            ...
            detail = await self._ingestion.get_inverter_detail(
                site_id, inv.inverter_id
            )
            if not detail:
                continue
            ...
            for mppt_num, mppt_strings in mppt_groups.items():
                ...
                for s in mppt_strings:
                    ...
                    current_dev = (
                        (s.dc_current_a - mean_current) / mean_current
                        if mean_current > 0 else 0
                    )
                    # Voltage deviation check
                    voltage_dev = (
                        (s.dc_voltage_v - mean_voltage) / mean_voltage
                        if mean_voltage > 0 else 0
                    )
                    # Power deviation check
                    power_dev = (
                        (s.dc_power_kw - mean_power) / mean_power
                        if mean_power > 0 else 0
                    )

                    # --- String open circuit: near-zero current, normal voltage ---
                    if (
                        s.dc_current_a < mean_current * 0.1
                        and s.dc_voltage_v > mean_voltage * 0.8
                    ):
                        anomalies.append(StringAnomaly(
                            ...
                            recommended_action=(
                                "Check string fuse, MC4 connectors, and "
                    ...
                    if (
                        voltage_dev < -STRING_VOLTAGE_DEVIATION_PCT
                        and abs(current_dev) < STRING_CURRENT_DEVIATION_PCT
                    ):
                        severity = (
                            "critical" if voltage_dev < -0.15 else "warning"
                        )
                        ...
                        anomalies.append(StringAnomaly(
                            ...
                            recommended_action=(
                                "IR scan panels on this string to identify "
                                "hot spot; check bypass diodes"
                            ),
                        ))
                        continue
                    ...
                    if power_dev < -STRING_POWER_DEVIATION_PCT:
                        severity = (
                            "warning" if power_dev > -0.15 else "critical"
                        )
                        ...
                        anomalies.append(StringAnomaly(
                            ...
                            probable_cause=(
                                "Soiling, partial shading, or "
                                "connector degradation"
                            ),
                            recommended_action=(
                                "Inspect panels for soiling/shading; "
            ...
            if len(mppt_groups) >= 2:
                ...
                for mppt_num, mppt_strings in mppt_groups.items():
                    avg_p = statistics.mean(
                        [s.dc_power_kw for s in mppt_strings]
                    )
                    ...
                    for mppt_num, avg_p in mppt_avg_powers.items():
                        ...
                        if mppt_dev < -STRING_POWER_DEVIATION_PCT:
                            ...
                            anomalies.append(StringAnomaly(
                                ...
                                probable_cause=(
                                    "MPPT tracker hardware fault or "
                                    "systematic shading on tracker inputs"
                                ),
                                recommended_action=(
    ...
    async def get_diagnostic_summary(
        ...
        if pr.performance_ratio > 0 and pr.pr_rating in ("acceptable", "poor"):
            ...
            issues.append(DiagnosticIssue(
                ...
                detail=(
                    f"Site PR at {pr.performance_ratio*100:.1f}% "
                    f"(target {pr.pr_target*100:.1f}%)"
                ),
                probable_cause=(
                    "System-wide degradation, soiling accumulation, "
                    "or inverter efficiency loss"
                ),
                recommended_action=(
                    "Review inverter peer comparison for specific "
                    "underperformers; schedule panel cleaning"
                ),
                confidence=0.80,
        ...
        for comp in peer_comparisons:
            if comp.status != "normal":
                severity = (
                    "warning" if comp.status == "underperforming" else "critical"
                )
                issues.append(DiagnosticIssue(
                    severity=severity,
                    ...
                    detail=(
                        f"{abs(comp.deviation_pct)*100:.0f}% below "
                        f"{comp.peer_group} peer group mean"
                    ),
                    probable_cause=comp.probable_cause,
                    recommended_action=(
                        f"Inspect strings on {comp.inverter_id}, "
                        f"check for soiling or disconnection"
                    ),
        ...
        if bess:
            ...
            if bess.cell_imbalance_mv > 50:
                issues.append(DiagnosticIssue(
                    ...
                    detail=(
                        f"Cell imbalance {bess.cell_imbalance_mv:.0f}mV "
                        f"(threshold 50mV)"
                    ),
                    probable_cause=(
                        "Cell aging variance or BMS balancing failure"
                    ),
                    recommended_action=(
                        "Run BMS active balancing cycle; if persists, "
                        "inspect rack for degraded cells"
                    ),
                    confidence=0.75,
                ))
            ...
            if bess.soh_pct < 90:
                issues.append(DiagnosticIssue(
                    ...
                    recommended_action=(
                        "Plan capacity replacement; adjust dispatch to "
        ...
        summary = {
            ...
            "total_monthly_cost_impact_zar": round(
                sum(i.cost_impact_monthly_zar for i in issues), 0
            ),
        }
    ...
    def _estimate_pr_loss_cost(self, pr: PerformanceMetrics) -> float:
        ...
        lost_kwh_per_day = (
            pr.installed_capacity_kwp * pr.peak_sun_hours * pr_gap
        )
