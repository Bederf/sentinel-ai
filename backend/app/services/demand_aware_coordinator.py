...
class DemandAwareCoordinator:
        ...
        try:
            ...
            if urgency == "critical" or urgency == "warning":
                ...
                if ModuleType.SOLAR in active_module_types:
                    return await self._generate_tou_guidance(site_id=site_id, active_module_types=active_module_types)
    ...
    async def _generate_tou_guidance(
        ...
        try:
            ...
            return {
                ...
                "module_actions": [
                    {"module": "solar", "action": "follow_tou_schedule", "schedule": schedule.to_dict()}
                ],
    ...
    async def _get_available_actions(self, site_id: str, active_module_types: list[ModuleType]) -> dict[str, list[str]]:
        ...
        if ModuleType.ENERGY in active_module_types:
            available["energy"] = ["defer_pump_30min", "reduce_lift_30min", "reduce_compressor_50pct"]

        # Lighting module
        if ModuleType.LIGHTING in active_module_types:
            available["lighting"] = ["dim_to_80pct", "dim_to_60pct", "reduce_outdoor_lighting_50pct"]
    ...
    def _generate_rule_based_shaving(
        ...
        if ModuleType.ENERGY in active_module_types and required_reduction_kw > total_reduction_kw:
            remaining = required_reduction_kw - total_reduction_kw
            if remaining > 30:
                module_actions.append({"module": "energy", "action": "defer_pump_30min", "reduction_kw": 25})
