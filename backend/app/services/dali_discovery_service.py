"""DALI Discovery Service - Query DALI-2 device information from gateways.

Supports discovering device metadata from DALI-2 compatible gateways including:
- Tridonic Scenecom
- Philips Dynalite
- Helvar Router
...
class DALIDiscoveryService:
    ...
    async def get_gateway_info(self) -> DALIGatewayInfo | None:
    ...
    async def get_device_info(self, dali_line: int, dali_address: int) -> DALIDeviceInfo | None:
...
# Simulated discovery for local/testing use when gateway not available
class SimulatedDALIDiscovery:
    """Simulated DALI discovery for local seeded mode."""
