...
class FireHVACCoordinator:
    ...
    async def _execute_hvac_action(self, target_id: str, action: str, dm=None) -> dict[str, Any]:
        ...
        if action == "shutdown":
            if dm:
                try:
                    await dm.write_device_value(target_id, "supply_fan", False, priority=1, user="fire_system")
    ...
    async def _execute_pressurization_action(self, target_id: str, action: str) -> dict[str, Any]:
        ...
        if action == "activate":
            ...
            self._repo.update_pressurization(
                target_id,
                {
    ...
    async def _execute_exhaust_action(self, target_id: str, action: str, dm=None) -> dict[str, Any]:
        ...
        if action == "activate":
            if dm:
                try:
                    await dm.write_device_value(target_id, "fan_speed", 60, priority=1, user="fire_system")
    ...
    async def shutdown_hvac_for_zone(self, zone_id: str) -> list[dict[str, Any]]:
        ...
        if dm:
            ...
            floor_devices = [
                ...
                if d.device_type.value in hvac_types and getattr(d, "location", {}).get("floor", "") == floor
            ]
            ...
            for device in floor_devices:
                try:
                    ...
                    if "supply_fan" in device.points:
                        await dm.write_device_value(device.id, "supply_fan", False, priority=1, user="fire_system")
    ...
    async def reset_fire_mode(self) -> dict[str, Any]:
        ...
        closed_dampers = [d for d in all_dampers if d.get("status") == DamperStatusEnum.CLOSED.value]
        ...
        for device_info in self._shutdown_devices:
            ...
            if "AHU" in device_id:
                ...
                if dm and not device_info.get("simulated"):
                    try:
                        await dm.write_device_value(device_id, "supply_fan", True, priority=1, user="fire_system")
                    except Exception as e:
        ...
        running_fans = [p for p in pressurization if p.get("fan_status") == FanStatus.RUNNING.value]
