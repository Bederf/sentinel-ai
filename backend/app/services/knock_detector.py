...
class KnockDetector:
    ...
    def analyze(
        ...
        peak_amps = spectrum_data.get(
            'peak_amplitudes_db',
            spectrum_data.get('peak_amplitudes', [])
        )
        ...
        # Get engine profile
        profile = self.ENGINE_KNOCK_PROFILES.get(
            engine_type,
            self.ENGINE_KNOCK_PROFILES['generator']
        )
        knock_range = profile['knock_range']

        # Look for low frequency peaks (engine knock signature)
        low_freq_peaks = [
            (f, a) for f, a in zip(peak_freqs, peak_amps)
            if knock_range[0] <= f <= knock_range[1]
        ]
        ...
        for knock_freq, knock_amp in low_freq_peaks:
            ...
            if harmonic_evidence['count'] >= 2:  # At least 2 harmonics
                confidence = self._calculate_knock_confidence(
                    knock_amp, peak_amps, harmonic_evidence
                )
        ...
        if best_knock and best_knock['confidence'] > 0.3:
            ...
            result['knock_type'] = self._classify_knock_type(
                best_knock['frequency'], profile
            )
    ...
    def _calculate_knock_confidence(
        ...
        if harmonic_evidence['harmonics']:
            ...
            if len(harmonic_amps) >= 2:
                ...
                decay_count = sum(
                    1 for i in range(len(harmonic_amps) - 1)
                    if harmonic_amps[i] >= harmonic_amps[i + 1]
                )
    ...
    def analyze_severity(
        ...
        if not knock_result.get('knock_detected'):
            return {
                'severity': 'none',
                'recommendation': 'No knock detected - continue normal operation'
            }
