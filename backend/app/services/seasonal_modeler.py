...
SA_SEASONAL_DATA = {
    1: SeasonalConfig(
        ...
        ambient_temp_min=18, ambient_temp_max=29, ambient_temp_avg=24,
        rainfall_probability=0.35, rainfall_avg_days=12, avg_rainfall_mm=85,
        cloud_cover_percent=0.45, solar_efficiency=0.75, daylight_hours=14.5,
        school_holidays=True,  # Still in summer break (until ~Jan 15)
        summer_break=True,
        occupancy_variance=0.20,  # 20% lower: summer break
        hvac_load_percent=95, fault_probability_multiplier=1.8,
        public_holidays=1  # New Year's Day (Jan 1)
    ),
    2: SeasonalConfig(
        month=2, month_name="February",
        season=Season.SUMMER,
        ambient_temp_min=18, ambient_temp_max=28, ambient_temp_avg=23,
        rainfall_probability=0.30, rainfall_avg_days=10, avg_rainfall_mm=75,
        cloud_cover_percent=0.40, solar_efficiency=0.80, daylight_hours=14.0,
        school_holidays=False, summer_break=False,
        occupancy_variance=-0.05,  # 5% lower: tail end of summer
        hvac_load_percent=90, fault_probability_multiplier=1.6,
        public_holidays=1  # Human Rights Day (Feb 21)
    ),
    3: SeasonalConfig(
        month=3, month_name="March",
        season=Season.AUTUMN,
        ambient_temp_min=16, ambient_temp_max=26, ambient_temp_avg=21,
        rainfall_probability=0.20, rainfall_avg_days=7, avg_rainfall_mm=60,
        cloud_cover_percent=0.35, solar_efficiency=0.85, daylight_hours=13.0,
        school_holidays=True,  # Easter break (varies)
        summer_break=False,
        occupancy_variance=0.15,  # 15% lower: Easter holiday
        hvac_load_percent=70, fault_probability_multiplier=1.1,
        public_holidays=2  # Good Friday, Family Day
    ),
    4: SeasonalConfig(
        month=4, month_name="April",
        season=Season.AUTUMN,
        ambient_temp_min=14, ambient_temp_max=24, ambient_temp_avg=19,
        rainfall_probability=0.15, rainfall_avg_days=5, avg_rainfall_mm=40,
        cloud_cover_percent=0.30, solar_efficiency=0.90, daylight_hours=12.5,
        school_holidays=False, summer_break=False,
        occupancy_variance=0.0,  # Normal
        hvac_load_percent=60, fault_probability_multiplier=0.9,
        public_holidays=1  # Workers Day (May 1 - near)
    ),
    5: SeasonalConfig(
        month=5, month_name="May",
        season=Season.AUTUMN,
        ambient_temp_min=12, ambient_temp_max=22, ambient_temp_avg=17,
        rainfall_probability=0.12, rainfall_avg_days=4, avg_rainfall_mm=35,
        cloud_cover_percent=0.28, solar_efficiency=0.92, daylight_hours=12.0,
        school_holidays=False, summer_break=False,
        occupancy_variance=0.0,  # Normal
        hvac_load_percent=55, fault_probability_multiplier=0.8,
        public_holidays=1  # Workers Day (May 1)
    ),
    6: SeasonalConfig(
        month=6, month_name="June",
        season=Season.WINTER,
        ambient_temp_min=8, ambient_temp_max=18, ambient_temp_avg=13,
        rainfall_probability=0.08, rainfall_avg_days=3, avg_rainfall_mm=25,
        cloud_cover_percent=0.25, solar_efficiency=0.95, daylight_hours=11.5,
        school_holidays=True,  # School break (Jun 27 - Jul 6)
        summer_break=False,
        occupancy_variance=-0.25,  # 25% lower: school holidays
        hvac_load_percent=30, fault_probability_multiplier=0.7,
        public_holidays=1  # Youth Day (Jun 16)
    ),
    7: SeasonalConfig(
        month=7, month_name="July",
        season=Season.WINTER,
        ambient_temp_min=7, ambient_temp_max=17, ambient_temp_avg=12,
        rainfall_probability=0.10, rainfall_avg_days=3, avg_rainfall_mm=28,
        cloud_cover_percent=0.28, solar_efficiency=0.94, daylight_hours=11.5,
        school_holidays=True,  # Winter break continues (Jun 27 - Jul 6)
        summer_break=False,
        occupancy_variance=-0.20,  # 20% lower: school holidays (first week)
        hvac_load_percent=25, fault_probability_multiplier=0.6,
        public_holidays=0  # No public holidays in July
    ),
    8: SeasonalConfig(
        month=8, month_name="August",
        season=Season.WINTER,
        ambient_temp_min=8, ambient_temp_max=19, ambient_temp_avg=13,
        rainfall_probability=0.12, rainfall_avg_days=4, avg_rainfall_mm=32,
        cloud_cover_percent=0.30, solar_efficiency=0.90, daylight_hours=12.0,
        school_holidays=False, summer_break=False,
    ...
    9: SeasonalConfig(
        ...
        ambient_temp_min=11, ambient_temp_max=23, ambient_temp_avg=17,
        rainfall_probability=0.15, rainfall_avg_days=5, avg_rainfall_mm=40,
        cloud_cover_percent=0.32, solar_efficiency=0.88, daylight_hours=12.5,
        school_holidays=False, summer_break=False,
        occupancy_variance=0.0,  # Normal
        hvac_load_percent=50, fault_probability_multiplier=0.8,
        public_holidays=1  # Heritage Day (Sep 24)
    ),
    10: SeasonalConfig(
        month=10, month_name="October",
        season=Season.SPRING,
        ambient_temp_min=14, ambient_temp_max=25, ambient_temp_avg=20,
        rainfall_probability=0.18, rainfall_avg_days=6, avg_rainfall_mm=50,
        cloud_cover_percent=0.35, solar_efficiency=0.85, daylight_hours=13.0,
        school_holidays=False, summer_break=False,
        occupancy_variance=0.0,  # Normal
        hvac_load_percent=65, fault_probability_multiplier=0.9,
        public_holidays=0  # No public holidays
    ),
    11: SeasonalConfig(
        month=11, month_name="November",
        season=Season.SPRING,
        ambient_temp_min=17, ambient_temp_max=28, ambient_temp_avg=22,
        rainfall_probability=0.25, rainfall_avg_days=8, avg_rainfall_mm=65,
        cloud_cover_percent=0.38, solar_efficiency=0.82, daylight_hours=13.5,
        school_holidays=False, summer_break=False,
        occupancy_variance=0.0,  # Normal
        hvac_load_percent=80, fault_probability_multiplier=1.1,
        public_holidays=0  # No public holidays
    ),
    12: SeasonalConfig(
        month=12, month_name="December",
        season=Season.SUMMER,
        ambient_temp_min=19, ambient_temp_max=30, ambient_temp_avg=25,
        rainfall_probability=0.35, rainfall_avg_days=12, avg_rainfall_mm=85,
        cloud_cover_percent=0.42, solar_efficiency=0.78, daylight_hours=14.5,
        school_holidays=True,  # School break (Dec 16 - Jan 15)
        summer_break=True,
        occupancy_variance=-0.25,  # 25% lower: Christmas/New Year holidays
        hvac_load_percent=100, fault_probability_multiplier=1.9,
