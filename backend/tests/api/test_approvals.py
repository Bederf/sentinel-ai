...
def approval_service():
    ...
    from app.services.quality_gate_policy import GateStatus, QualityGateResult
    ...
    # Quality gate: always pass in unit tests (no real DB)
    from app.services.quality_gate_policy import EnforcementAction
    ...
    service._check_quality_gate = AsyncMock(return_value=passing_gate)
    return service
...
class TestApprovalExecution:
    ...
    async def test_approve_recommendation_success(self, approval_service, mock_recommendation):
        ...
        with (
            patch("app.services.approval_service.settings", mock_settings),
            patch("app.models.onboarding_phase.effective_phase", AsyncMock(return_value="supervised")),
            patch("app.services.approval_service.ParasiteDecisionRepository") as mock_pdr,
            patch("app.services.approval_service.emit_decision_event"),
            patch("app.services.execution_service.execute_command", new=AsyncMock(return_value=exec_result)),
        ):
            mock_pdr.return_value.record_decision = AsyncMock()
