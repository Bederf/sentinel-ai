...
class TestAPIKeySupabaseDowntime:
    ...
    @pytest.mark.asyncio
    async def test_api_key_validation_supabase_down_returns_none(self):
        ...
        with patch("app.middleware.auth_middleware.get_supabase_client") as mock_get_client:
            ...
            result = await _validate_api_key("sent_sk_test_key_12345")
    ...
    @pytest.mark.asyncio
    async def test_api_key_validation_supabase_timeout_returns_none(self):
    ...
    @pytest.mark.asyncio
    async def test_api_key_validation_supabase_auth_error_returns_none(self):
    ...
    @pytest.mark.asyncio
    async def test_api_key_validation_supabase_generic_error_returns_none(self):
    ...
    @pytest.mark.asyncio
    async def test_api_key_validation_no_in_memory_fallback(self):
    ...
    @pytest.mark.asyncio
    async def test_api_key_validation_valid_supabase_response(self):
        """Valid Supabase response returns key info correctly."""
        with patch("app.middleware.auth_middleware.get_supabase_client") as mock_get_client:
            mock_client = mock_get_client.return_value
            table_result = mock_client.table.return_value
            select_result = table_result.select.return_value
            eq_result = select_result.eq.return_value
            limit_result = eq_result.limit.return_value
            mock_query = limit_result.execute.return_value
    ...
    @pytest.mark.asyncio
    async def test_api_key_validation_revoked_key_returns_none(self):
    ...
    @pytest.mark.asyncio
    async def test_api_key_validation_no_cache_after_supabase_error(self):
        """Supabase errors should not populate any cache."""
        with patch("app.middleware.auth_middleware.get_supabase_client") as mock_get_client:
            mock_get_client.side_effect = Exception("Database unavailable")

            # Call validation
            result1 = await _validate_api_key("sent_sk_test_key_12345")
            ...
            result2 = await _validate_api_key("sent_sk_test_key_12345")
