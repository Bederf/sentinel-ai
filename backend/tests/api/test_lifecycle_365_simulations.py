            ...
            "recent_events": [
                {
                    ...
                    "simulated_hour": 6,
                    "description": "Building occupancy increases",
                    "details": {"occupancy_percent": 35},
                },
                {
                    "event_type": "setpoint_change",
                    "simulated_hour": 8,
                    "description": "HVAC setpoint adjusted",
                    "details": {"new_setpoint": 21},
                },
                {
                    "event_type": "ai_optimization",
                    "simulated_hour": 10,
                    "description": "AI optimization recommended",
                    "details": {"recommendations": [{"type": "hvac", "savings": "5%"}]},
                },
            ],
            "active_faults": {},
            "pending_repairs": {},
...
class TestHourlyEventProcessing:
    ...
    @pytest.mark.asyncio
    async def test_daily_event_count(
        self,
        async_client: AsyncClient,
        hvac_dali_ai_scenario: Dict[str, Any]
    ):
        ...
        if days_elapsed > 0:
            ...
            assert 7 <= avg_events_per_day <= 10, \
                f"Expected ~8 events/day, got {avg_events_per_day:.1f}"

    @pytest.mark.asyncio
    async def test_event_type_variety(
        self,
        async_client: AsyncClient,
        hvac_dali_ai_scenario: Dict[str, Any]
    ):
...
class TestSeasonalVariations:
    ...
    @pytest.mark.asyncio
    async def test_temperature_cycle(
        self,
        async_client: AsyncClient,
        hvac_dali_ai_scenario: Dict[str, Any]
    ):
        ...
        if ambient_temp is not None:
            ...
            assert 10 <= ambient_temp <= 35, \
                f"Temperature {ambient_temp}°C outside expected seasonal range"

    @pytest.mark.asyncio
    async def test_seasonal_name_changes(
        self,
        async_client: AsyncClient,
        hvac_dali_ai_scenario: Dict[str, Any]
    ):
        ...
        daily_summaries = [
            e for e in events 
            if e.get("event_type") == "daily_summary"
        ]
        ...
        assert len(found_seasons) >= 2, \
            f"Expected multiple seasons in simulation, found: {found_seasons}"

    @pytest.mark.asyncio
    async def test_rainfall_pattern(
        self,
        async_client: AsyncClient,
        hvac_dali_ai_scenario: Dict[str, Any]
    ):
        ...
        if cloud_cover is not None:
            assert 0 <= cloud_cover <= 100, \
                f"Cloud cover {cloud_cover}% outside expected range"
...
class TestAIRecommendations:
    ...
    async def test_ai_optimization_event_generation(
        ...
        optimization_events = [
            e for e in events 
            if e.get("event_type") == "ai_optimization"
        ]
        ...
        for expected_hour in expected_hours:
            assert expected_hour in optimization_hours, \
                f"Missing ai_optimization event at hour {expected_hour}"
    ...
    async def test_recommendation_payload_validity(
        ...
        optimization_events = [
            e for e in events 
            if e.get("event_type") == "ai_optimization" and e.get("details")
        ]
...
class TestCheckpointRecovery:
    ...
    async def test_pause_resume_preserves_state(
        ...
        assert time_after > time_before or events_after > events_before, \
            "Simulation did not progress after resume"
...
class TestProgressTracking:
    ...
    async def test_progress_percentage_accuracy(
        ...
        for _ in range(5):
            ...
            assert 0 <= progress <= 100, \
                f"Progress {progress}% outside valid range"
            
            # Progress should increase or stay same (never decrease)
            assert progress >= previous_progress, \
                f"Progress decreased from {previous_progress}% to {progress}%"
    ...
    async def test_progress_reaches_completion(
        ...
        assert progress >= 90 or is_complete, \
            f"Simulation incomplete: {progress}% done"
...
class TestGrantHVACAIScenario:
    ...
    async def test_hvac_setpoint_recommendations(
        ...
        setpoint_events = [
            e for e in events 
            if "setpoint" in e.get("description", "").lower() 
            or e.get("event_type") == "setpoint_change"
        ]
        
        assert len(setpoint_events) > 0, \
            "No HVAC setpoint change events found in simulation"
    ...
    async def test_dali_lighting_recommendations(
        ...
        dali_events = [
            e for e in events 
            if any(term in e.get("description", "").lower() 
                   for term in ["dali", "lighting", "lux", "daylight"])
        ]
        
        # DALI events should be present in at least some days
        assert len(dali_events) > 0, \
            "No DALI lighting events found in simulation"
...
class TestGrantSolarBESSScenario:
    ...
    @pytest.mark.asyncio
    async def test_solar_dispatch_events(
        self,
        async_client: AsyncClient,
        solar_bess_ai_scenario: Dict[str, Any]
    ):
        ...
        solar_events = [
            e for e in events 
            if any(term in e.get("description", "").lower() 
                   for term in ["solar", "bess", "battery", "dispatch"])
        ]
...
class TestSimulationStress:
    ...
    async def test_simulation_completes_within_time_budget(
        ...
        progress = status_data.get("progress_percent", 0)
        assert progress > 90, \
            f"Simulation only {progress}% complete after {elapsed:.1f} seconds"
        ...
        assert elapsed < max_allowed, \
            f"Simulation took {elapsed:.1f}s, exceeded max {max_allowed}s"
    ...
    async def test_concurrent_simulations(
        ...
        hvac_progress = hvac_status.json().get("progress_percent", 0)
        solar_progress = solar_status.json().get("progress_percent", 0)
...
class TestSimulationErrorHandling:
    ...
    async def test_invalid_scenario_rejected(self, async_client: AsyncClient):
        ...
        assert response.status_code in (400, 404), \
            f"Expected 400/404 for invalid scenario, got {response.status_code}"
    ...
    async def test_status_for_nonexistent_task(self, async_client: AsyncClient):
        ...
        assert response.status_code in (200, 404), \
            f"Expected 200/404 for nonexistent task, got {response.status_code}"
