/**
 * Optimized CloudFlare Worker with Edge-First Caching
...
// Debug mode control
const DEBUG_HEADERS = true; // Set to false to remove debug headers
  ...
  async fetch(
    ...
    if (cacheHit) {
      ...
      if (DEBUG_HEADERS) {
        ...
        response.headers.set('X-Worker', 'aimthelaw-opt-prod-v1');
      }
    ...
    if (neg) {
      const headers = { ...MINIMAL_SECURITY_HEADERS };
      if (DEBUG_HEADERS) {
        headers['X-Cache'] = 'NEG-HIT';
        headers['X-Worker'] = 'aimthelaw-opt-prod-v1';
      }
      headers['Server-Timing'] = 'phase;desc="NEG-HIT"';
    ...
    if (originRes.status >= 200 && originRes.status < 300) {
      ...
      // Log negative cache for monitoring
    ...
    if (DEBUG_HEADERS) {
      headers.set('X-Cache', 'BYPASS-METHOD');
      headers.set('X-Worker', 'aimthelaw-opt-prod-v1');
    }
