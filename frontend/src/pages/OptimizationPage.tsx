...
export function OptimizationPage({ onError }: OptimizationPageProps) {
  ...
  const [activeTab, setActiveTab] = useState("load-shedding");
    ...
    <div className="h-full overflow-y-auto p-4 md:p-6" style={{ background: "var(--color-sentinel-bg-canvas)" }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KPICard
          ...
          value={`${kpis.energySavings}%`}
          icon={<TrendingDown className="h-5 w-5" />}
          accentColor="green"
          subtitle="avg. across sites"
          density="compact"
        />

        <KPICard
          title="Comfort Extension"
          value={`${kpis.comfortExtension} min`}
          icon={<Thermometer className="h-5 w-5" />}
          accentColor="blue"
          subtitle="avg. extension"
          density="compact"
        />

        <KPICard
          title="Fuel Savings"
          value={`${kpis.fuelSavings}%`}
          icon={<Zap className="h-5 w-5" />}
          accentColor="orange"
          subtitle="avg. fuel savings"
          density="compact"
        />

        <KPICard
          title="Cost Savings"
          value={formatZAR(kpis.costSavings)}
          icon={<DollarSign className="h-5 w-5" />}
          accentColor="green"
          subtitle="avg. per outage"
          density="compact"
        />
      </div>
            ...
            <div className="space-y-3">
              {actionHistory.length === 0 && (
                <EmptyState
                  ...
                  subtext="Optimization actions will appear here once executed."
                />
              )}
