...
/** True when the cockpit has a real dominant narrative from the backend (not equipment-list fallback) */
function hasDominantNarrative(state: CockpitState): boolean {
  // activeCondition.summary is the dominant narrative only when it does NOT
  // look like the equipment-warning fallback text generated by mapCockpitState
  const s = state.activeCondition.summary
  if (!s) return false
  if (s.startsWith('Building is calm')) return false
  if (/^\d+ equipment (at health warning|with low health)/.test(s)) return false
  if (s === 'Awaiting building signal') return false
  return true
}
...
function heroHeadline(state: CockpitState) {
  ...
  if (posture && posture !== 'Waiting') return `${prefix}${posture}`
  return `${prefix}Building within operating margin`
}
...
function buildQueue(state: CockpitState): QueueItem[] {
  ...
  // Dominant narrative with no secondary tensions
    ...
    return [
      {
        ...
        impact: `${modules} remains the primary focus.`,
  ...
  // No dominant narrative — show posture-based calm signal, not equipment list
  const posture = state.site.posture ?? 'Drifting — safe bounds'
  return [
    {
      severity: 'Monitor',
      title: `${posture} — no dominant signal`,
      cause: `${state.site.name} is operating within margin. No cross-system drift pattern has emerged above the background threshold.`,
      impact: `${modules} — equipment health data is visible in the Equipment tab.`,
      eta: 'No immediate breach',
      confidence: evidenceLabel(state),
    },
  ]
}
...
function buildForecast(state: CockpitState) {
  ...
  } else if (state.emergingRisks.length > 0) {
    modulesValue = state.emergingRisks[0]?.detail ?? state.decision.tradeoff
  } else {
    modulesValue = `${modules} stable — no secondary tensions rising above background`
  }
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
        ...
        >
          ...
          {issuesPayload !== undefined ? (
            ...
            <>
              <DetailRow icon={<Brain className="h-3.5 w-3.5" />} label="Observation" value={state.activeCondition.summary} accent />
              <DetailRow icon={<Layers3 className="h-3.5 w-3.5" />} label="ML Training" value={state.emergingRisks.length > 0 ? `${state.emergingRisks.length} tension(s) under observation` : 'Model calibration in progress'} />
              <DetailRow icon={<Clock3 className="h-3.5 w-3.5" />} label="Observation window" value="Ongoing — no constraint breach predicted" accent />
              <DetailRow icon={<ArrowRight className="h-3.5 w-3.5" />} label="Mode" value={`Observation only · ${state.site.dataFreshnessLabel}`} accent />
              <DetailRow icon={<Gauge className="h-3.5 w-3.5" />} label="Load profile" value={telemetrySnapshot(state)} />
            </>
          ) : (
            <>
              <DetailRow icon={<Brain className="h-3.5 w-3.5" />} label="Cause" value={waiting ? 'No live building signal yet.' : (hasDominantNarrative(state) ? state.activeCondition.summary : `${state.site.name} is operating within margin.`)} accent />
              ...
              <DetailRow icon={<Gauge className="h-3.5 w-3.5" />} label="Trade-off" value={waiting ? 'No intervention energy penalty while waiting.' : (hasDominantNarrative(state) ? `${state.decision.tradeoff} · ${telemetrySnapshot(state)}` : `${telemetrySnapshot(state)} · No dominant pattern active`)} />
            </>
          )}
