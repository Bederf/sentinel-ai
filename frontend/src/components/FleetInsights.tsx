...
export function FleetInsights() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const [sumRes, patRes, riskRes, benchRes, gmRes, ftRes, impRes] =
        await Promise.all([
          fleetApi.getSummary(),
          fleetApi.getFailurePatterns(),
          fleetApi.getRiskDistribution(),
          fleetApi.getBenchmarks(),
          fleetApi.getGlobalModels(),
          fleetApi.getFineTunedModels(),
          fleetApi.getImprovementSummary(),
        ]);
      setSummary(sumRes);
