...
################################################################################
# Rename Script: Sentry → SENTRY (Simplified Version)
#
# Usage:
#   ./rename_sentry_to_sentry_v2.sh --dry-run
#   ./rename_sentry_to_sentry_v2.sh --execute
#   ./rename_sentry_to_sentry_v2.sh --rollback
#
################################################################################
...
find_sentry_files() {
  find "$PROJECT_ROOT" -type f \
    ...
    -print0 2>/dev/null | xargs -0 grep -l "sentry\|sentry\|moltbot" 2>/dev/null | sort -u
}
...
dry_run() {
  ...
  if [[ -z "$files" ]]; then
    warn "No sentry references found"
    return 0
  fi
...
main() {
  ...
  echo "╔════════════════════════════════════════════╗" | tee -a "$LOG_FILE"
  echo "║ Sentry → SENTRY Rename                   ║" | tee -a "$LOG_FILE"
