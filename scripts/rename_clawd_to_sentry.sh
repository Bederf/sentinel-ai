...
################################################################################
# Rename Script: OpenClaw → SENTRY
#
# This script systematically renames all clawd/openclaw references to sentry
# across the codebase. It supports dry-run mode and rollback.
#
# Usage:
#   ./scripts/rename_clawd_to_sentry.sh --dry-run     # Preview changes
#   ./scripts/rename_clawd_to_sentry.sh --execute     # Perform renaming
#   ./scripts/rename_clawd_to_sentry.sh --rollback    # Undo changes
#
################################################################################
...
declare -A PATTERNS=(
  ...
  ["Clawd service"]="Sentry service"
  ["Clawd API"]="Sentry API"
...
# Find all files that contain clawd references
find_clawd_files() {
  local exclude_args
  exclude_args=$(build_exclude_args)

  # Use grep to find files containing clawd references (case-insensitive for some)
  find "$PROJECT_ROOT" \
    -type f \
    \( -name "*.py" -o -name "*.ts" -o -name "*.tsx" -o -name "*.md" \
       -o -name "*.yaml" -o -name "*.yml" -o -name "*.json" -o -name "*.sh" \) \
    $exclude_args \
    -exec grep -l -i "clawd\|openclaw\|moltbot" {} \; 2>/dev/null | sort -u
}
...
dry_run() {
  ...
  if [[ -z "$files" ]]; then
    warn "No files containing clawd/openclaw references found"
    return 0
  fi
...
refactor(bot): Rename OpenClaw → SENTRY system-wide
...
show_help() {
  cat << 'EOF'
OpenClaw → SENTRY Rename Script

USAGE:
  ./scripts/rename_clawd_to_sentry.sh [OPTION]
...
main() {
  ...
  echo "╔════════════════════════════════════════════════════════════╗" | tee -a "$LOG_FILE"
  echo "║  OpenClaw → SENTRY Rename Script                          ║" | tee -a "$LOG_FILE"
  ...
  case "$mode" in
    ...
    --execute)
      ...
      echo "  git add -A"
      echo "  git commit -m 'refactor(bot): Rename OpenClaw → SENTRY system-wide'"
      ;;
    --rollback)
      rollback
      ;;
