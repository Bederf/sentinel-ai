...
| AUTH-003 | Brute-Force Login Protection | App-level Redis tracker + fail2ban OS-level escalation | `api/auth.py` lines 105-180 (`LoginAttemptTracker`) + `infrastructure/fail2ban/jail.local` | App-level: Redis sorted sets (`ZADD`/`ZCOUNT`/`ZREMRANGEBYSCORE`) with TTL; fail2ban: `sentinel-api` jail monitors `/var/log/sentinel/security.log` | App-level: 5 failures → 15-min lockout (shared across gunicorn workers, Phase 209); fail2ban OS-level: 10 violations/5min → 15min ban, `recidive`: 50 violations/24h → 24h ban |
...
| ~~**HIGH**~~ CLOSED | ~~Optimization API path not routed through ApprovalService~~ | `api/optimization.py` | Fixed 2026-03-28 (Phase 172 P2): `Recommendation` record persisted before routing; `RoutingDecision → TierRoutingResult` adapter wired; Tier 3 routes through `ApprovalService.auto_execute_recommendation()`; `routing_source="optimization_api"` in success-path `parasite_decisions`; 284 tests pass | APPROVAL-001, SAFETY-003 | See commits 172-09/10/11/12 |
| **LOW** | Operator direct-write parity — `approve_optimization`, precooling endpoints | `api/optimization.py:1084,1584,1686` | Operator-initiated paths still call `device_manager.write_device_value()` directly; not shadow autonomy, but inconsistent with execution boundary | SAFETY-003 | Fold into next phase that touches these endpoints |
| **LOW** | Missing `routing_source` on failure-path `parasite_decisions` rows | `services/approval_service.py:1009,1092` | Safety-blocked and write-failed failure records do not carry `routing_source`; audit is mostly intact but failure traces are less complete | ISO-A.8.2 | Targeted fix — 2 call sites, low risk |
